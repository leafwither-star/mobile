...
(function injectUltimateFinalSystem() {
    ...
    if (!document.getElementById(styleId)) {
        ...
        style.innerHTML = `
            ...
            .beautiful-packet {
                ...
                min-width: 195px !important; max-width: 220px !important; cursor: pointer; display: block !important;
                box-shadow: 0 4px 12px rgba(250,158,59,0.3) !important; font-size: 14px !important;
                position: relative; z-index: 99; text-align: left !important; line-height: 1.4 !important;
            }
            ...
            /* iOS 弹窗样式 */
            .ios-banner { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); width: 90%; max-width: 350px; background: rgba(255,255,255,0.9); backdrop-filter: blur(15px); border-radius: 20px; padding: 12px 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); z-index: 999999; display: flex; align-items: center; transition: all 0.5s cubic-bezier(0.18, 0.89, 0.32, 1.28); opacity: 0; pointer-events: none; }
            .ios-banner.show { opacity: 1; top: 30px; }
        `;
        document.head.appendChild(style);
    }
    ...
    const getFriendsData = () => {
        ...
        let fullText = "";
        chatLog.forEach(e => { if((e.mes||"").includes('[手机快讯]')) fullText += e.mes + "\n"; });
        ...
        CLOUD_IDS.forEach(fId => {
            ...
            for (let j = lines.length - 1; j >= 0; j--) {
                if (lines[j].includes(`|${fId}|`)) {
                    ...
                    item.messageIndex = j + (item.hasUnreadTag ? 1000000 : 0);
    ...
    let lastTopMsgKey = "";
    const checkNotifications = (topFriend) => {
        if (!topFriend || topFriend.lastMessage === "暂无消息") return;
        const currentKey = `${topFriend.number}_${topFriend.lastMessage}`;
        if (lastTopMsgKey && lastTopMsgKey !== currentKey && topFriend.hasUnreadTag) {
            bubbleSound.play().catch(()=>{});
            const banner = document.createElement('div');
            banner.className = 'ios-banner';
            banner.innerHTML = `<div style="width:40px; height:40px; background:#4cd964; border-radius:10px; margin-right:12px; display:flex; align-items:center; justify-content:center; color:white; font-size:20px;">💬</div><div><div style="font-weight:bold; color:#000;">${topFriend.name}</div><div style="font-size:13px; color:#333;">${topFriend.lastMessage}</div></div>`;
            document.body.appendChild(banner);
            setTimeout(() => banner.classList.add('show'), 100);
            setTimeout(() => { banner.classList.remove('show'); setTimeout(() => banner.remove(), 500); }, 4000);
        }
        lastTopMsgKey = currentKey;
    };
    ...
    setInterval(() => {
        ...
        if (!window.friendRenderer.isPatched) {
            window.friendRenderer.extractFriendsFromContext = getFriendsData;
            window.friendRenderer.isPatched = true;
        }

        const data = getFriendsData();
        checkNotifications(data[0]); // 检查弹窗
        ...
        document.querySelectorAll('.message-item').forEach(item => {
            const id = item.getAttribute('data-friend-id');
            const f = data.find(d => d.number === id);
            if (!f) return;

            if (!item.querySelector('.custom-timestamp')) {
                const s = document.createElement('span'); s.className = 'custom-timestamp';
                item.appendChild(s);
            }
            item.querySelector('.custom-timestamp').innerText = f.lastMessageTime;

            const oldDot = item.querySelector('.unread-dot');
            if (f.hasUnreadTag && !oldDot) {
                const d = document.createElement('div'); d.className = 'unread-dot';
                item.appendChild(d);
            } else if (!f.hasUnreadTag && oldDot) {
                oldDot.remove();
            }
        });
    ...
    window.launchPerfectPacket = (wish, amount) => {
        ...
        overlay.innerHTML = `<div class="packet-dialog"><div style="position:absolute; top:20px; right:20px; font-size:30px; cursor:pointer;" onclick="document.getElementById('perfect-overlay').remove()">×</div><div style="margin-top:60px; opacity:0.8;">来自好友的红包</div><div style="margin-top:30px; font-size:22px; font-weight:bold; padding:0 20px; text-align:center;">${wish}</div><div id="p-open-target" class="open-btn-anim">開</div></div>`;
        document.body.appendChild(overlay);
        document.getElementById('p-open-target').onclick = function() {
            this.classList.add('spin');
            setTimeout(() => {
                const dialog = this.closest('.packet-dialog');
                dialog.innerHTML = `<div style="position:absolute; top:20px; right:20px; font-size:30px; cursor:pointer;" onclick="document.getElementById('perfect-overlay').remove()">×</div><div style="margin-top:100px; font-size:50px; font-weight:bold;">${amount}<span style="font-size:20px"> 元</span></div><div style="margin-top:20px; opacity:0.8;">已存入零钱</div><div style="margin-top:auto; margin-bottom:40px; border:1px solid #fbd69b; padding:4px 15px; border-radius:4px; font-size:13px;">查看领取详情 ></div>`;
            }, 600);
        };
    };
})();
